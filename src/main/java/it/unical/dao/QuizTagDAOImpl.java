...
public class QuizTagDAOImpl implements QuizTagDAO
{
	...
	{
		final Session session = databaseHandler.getSessionFactory().openSession();
		final List<String> tagValues = session.createQuery("select distinct(value) from QuizTag order by value asc")
				.list();
		session.close();
		return tagValues;
	}
	...
	public List<String> getAllTagValuesByQuiz(Integer idQuiz)
	{
		...
		final Query query = session
				.createQuery("select values from QuizTag T where T.quiz.id = :idQuiz order by value asc");
		query.setParameter("idQuiz", idQuiz);
	...
	public List<QuizTag> getByValue(String value)
	{
		...
		final List<QuizTag> quizTags = query.list();
		session.close();
		return quizTags;
	}
	...
	{
		final Session session = databaseHandler.getSessionFactory().openSession();
		final String hql = "SELECT value, COUNT(value) AS value_occurrence FROM QuizTag GROUP BY value ORDER BY value_occurrence DESC";
